...
// ✅ OPTIMIZED: Faster compression, better parallel processing, improved UX
...
// ✅ Lower quality + smaller maxWidth = faster upload, less payload
// ✅ PDFs are passed through as-is (no canvas overhead)
function compressImage(file, maxWidth = 700, quality = 0.60) {
...
// ─── Loading Overlay ──────────────────────────────────────────────────────────
// ✅ NEW: Shows real-time progress to user so they don't think it's stuck
function LoadingOverlay({ message, progress }) {
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center">
        <div className="w-16 h-16 mx-auto mb-4 relative">
          <svg className="animate-spin w-16 h-16 text-[#1a5c2a]" viewBox="0 0 24 24" fill="none">
            <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" strokeOpacity="0.2" />
            <path d="M12 2a10 10 0 0 1 10 10" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
          </svg>
          <span className="absolute inset-0 flex items-center justify-center text-xl">
            {progress < 33 ? '📦' : progress < 66 ? '⬆️' : progress < 90 ? '📄' : '✅'}
          </span>
        </div>
        <p className="text-[#1a5c2a] font-bold text-base mb-2">{message}</p>
        <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
          <div
            className="bg-[#1a5c2a] h-2 rounded-full transition-all duration-700 ease-out"
            style={{ width: `${progress}%` }}
          />
        </div>
        <p className="text-gray-400 text-xs mt-3">Kripya page band mat karein...</p>
      </div>
    </div>
  )
}
...
// ─── Payment Info Card ────────────────────────────────────────────────────────
function PaymentInfoCard({ feeGeneral, feeOBC, category }) {
    ...
    <div className="bg-[#f0f7f0] border border-[#4a9e5c] rounded-2xl p-4 mb-5">
      ...
      <div className="flex flex-col items-center gap-5">
        <div className="flex flex-col items-center gap-2">
          <div className="w-72 h-72 bg-white border-2 border-[#1a5c2a] rounded-xl flex items-center justify-center overflow-hidden">
            <img
...
export default function ApplyFormPage() {
  ...
  fetch(`${import.meta.env.VITE_BACKEND_URL}/api`).catch(() => {})
}, [])
  ...
  const validateStep2 = () => {
    ...
    if (!payment.senderUpiId?.trim()) { showToast('Please enter your UPI ID'); return false }
    if (payment.transactionId?.trim() && !/^[a-zA-Z0-9_\-]{8,}$/.test(payment.transactionId.trim())) {
  ...
  const pingUntilAwake = useCallback(async (maxWaitMs = 12000) => {
  ...
  // ─── OPTIMIZED Final Submit ───────────────────────────────────────────────────
  // ✅ Progress bar, parallel compression, async drive+email on backend
  const handleFinalSubmit = async () => {

    if (!validateStep2() || loading) return
    setLoading(true)

    const utr = payment.transactionId?.trim() || ''

    try {
      // 🔥 STEP 0: Wake-up Render server
      setLoadingMsg('Server start ho raha hai...')
      setLoadingProgress(5)

      // try {
      //   await fetch(`${import.meta.env.VITE_BACKEND_URL}/api`)
      //   // ⏳ little wait so server fully wake ho jaye
      //   await new Promise(res => setTimeout(res, 1500))
      // } catch (e) {
      //   console.log('Wake-up ping failed (ignore)', e)
      // }

      await pingUntilAwake()

      // ── Stage 1: Compress all files in parallel (fastest possible) ──
      setLoadingMsg('Documents compress ho rahe hain...')
      setLoadingProgress(10)

      // ✅ Tuned quality per file type for smallest payload:
      //    Photo/Signature — small, high quality
      //    Docs (aadhar, passbook, marksheets) — medium quality ok
      //    Screenshot — medium
      const [
        ...
        files.photo ? compressImage(files.photo, 600, 0.72) : Promise.resolve(null),
        files.signature ? compressImage(files.signature, 500, 0.75) : Promise.resolve(null),
        files.aadharDoc ? compressImage(files.aadharDoc, 1000, 0.70) : Promise.resolve(null),
        files.bankPassbook ? compressImage(files.bankPassbook, 1000, 0.70) : Promise.resolve(null),
        files.tenthDoc ? compressImage(files.tenthDoc, 1000, 0.70) : Promise.resolve(null),
        files.twelfthDoc ? compressImage(files.twelfthDoc, 1000, 0.70) : Promise.resolve(null),
        files.qualificationDoc ? compressImage(files.qualificationDoc, 1000, 0.70) : Promise.resolve(null),
        files.additionalDoc ? compressImage(files.additionalDoc, 1000, 0.70) : Promise.resolve(null),
        payment.screenshotFile ? compressImage(payment.screenshotFile, 900, 0.68) : Promise.resolve(null),
      ])

      setLoadingProgress(35)
      setLoadingMsg('Application submit ho rahi hai...')

      // ── Stage 2: Backend call ──
      ...
      setLoadingProgress(100)
      setLoadingMsg('Success! Redirect ho raha hai...')
        ...
        <div className="bg-white rounded-3xl shadow-lg p-6 sm:p-10 border-b-4 border-[#f0c020]">
          ...
          {step === 2 && (
            <>
              ...
              <PaymentInfoCard feeGeneral={post.feeGeneral} feeOBC={post.feeOBC} category={form.category} />
